...
#define PyGLM_ZERO_DIVISION_ERROR_T(T) if (std::numeric_limits<T>::is_iec559) { if (PyGLM_SHOW_WARNINGS & PyGLM_FLOAT_ZERO_DIVISION_WARNING) { PyErr_WarnEx(PyExc_UserWarning, "Uh oh.. There is a float division by zero here. I hope that's intended!\n(You can silence this warning using glm.silence(2))", 1); }} else { PyGLM_ZERO_DIVISION_ERROR; return NULL; }
