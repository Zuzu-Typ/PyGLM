...
tvec3_neg(tvec3 *obj)
{
	PyObject* argList = Py_BuildValue("ddd", -obj->x, -obj->y, -obj->z);
...
tvec3_abs(tvec3 *obj)
{
	PyObject* argList = Py_BuildValue("ddd", fabs(obj->x), fabs(obj->y), fabs(obj->z));
...
{
	char * out = (char*)malloc((60) * sizeof(char));
	snprintf(out,60, "tvec3( %10.10g, %10.10g, %10.10g )", self->x, self->y, self->z);
...
static double * unswizzle_tvec3(tvec3 * self, char c) {
	if (c == 'x' || c == 'r' || c == 's') {
		return &self->x;
	}
	if (c == 'y' || c == 'g' || c == 't') {
...
static PyObject * tvec3_getattr(PyObject * obj, PyObject * name) {
	...
	if (len == 1) {
		double *x = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[0]);
		if (x != NULL) {
		...
		double *x = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[0]);
		double *y = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[1]);
		if (x != NULL && y != NULL) {
		...
		double *x = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[0]);
		double *y = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[1]);
		double *z = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[2]);
		...
		double *y = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[1]);
		double *z = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[2]);
		double *w = unswizzle_tvec3((tvec3 *)obj, name_as_ccp[3]);
