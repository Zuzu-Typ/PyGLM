...
{
	char * out = (char*)malloc((86) * sizeof(char));
	snprintf(out, 86, "tmat2x2( ( %10.10g, %10.10g ), ( %10.10g, %10.10g ) )", self->x->x, self->x->y, self->y->x, self->y->y);
...
{
	char * out = (char*)malloc((84) * sizeof(char));
	snprintf(out, 84, "tmat2x2\n[ %10.10g | %10.10g ]\n[ %10.10g | %10.10g ]", self->x->x, self->x->y, self->y->x, self->y->y);
	return PyUnicode_FromString(out);
}
...
static int tmat2x2_contains(tmat2x2 * self, PyObject * value) {
	...
	return (int)((self->x->x == o->x && self->x->y == o->y) || (self->y->x == o->x && self->y->y == o->y));
}
...
static int tmat2x2_setattr(PyObject * obj, PyObject * name, PyObject * value) {
	...
	if (strcmp(name_as_ccp, "x") == 0) {
		...
		*((tmat2x2*)obj)->x = *((tvec2*)build_ivec2p(o));
		return 0;
	}
	if (strcmp(name_as_ccp, "y") == 0) {
		...
		*((tmat2x2*)obj)->y = *((tvec2*)build_ivec2p(o));
		return 0;
	}
	return PyObject_GenericSetAttr(obj, name, value);
}
