...
{
	tvec4 * temp = (tvec4*)tvec4_pow((PyObject*)self, obj1, obj2);
...
{
	char * out = (char*)malloc((75) * sizeof(char));
	snprintf(out, 75, "tvec4( %10.10g, %10.10g, %10.10g, %10.10g )", self->x, self->y, self->z, self->w);
...
static double * unswizzle_tvec4(tvec4 * self, char c) {
	if (c == 'x' || c == 'r' || c == 's') {
		return &self->x;
	}
	if (c == 'y' || c == 'g' || c == 't') {
...
static PyObject * tvec4_getattr(PyObject * obj, PyObject * name) {
	...
	if (len == 1) {
		double *x = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[0]);
		if (x != NULL) {
		...
		double *x = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[0]);
		double *y = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[1]);
		if (x != NULL && y != NULL) {
		...
		double *x = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[0]);
		double *y = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[1]);
		double *z = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[2]);
		...
		double *y = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[1]);
		double *z = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[2]);
		double *w = unswizzle_tvec4((tvec4 *)obj, name_as_ccp[3]);
